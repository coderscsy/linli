...
$timeUpdateTo = 'function z(e){const t=e.target;if(t!==i.value)return;const c=Number.isFinite(t.duration)&&t.duration>0?t.duration:0;c&&c!==a.value&&(a.value=c);const p=String(t.currentSrc||t.src||"");if(window.__OliviaSoulActivePlayerRevision&&window.__OliviaSoulActiveSongId&&window.__OliviaSoulActiveSessionId)return window.__OliviaSoulPlayerPost({commandRevision:window.__OliviaSoulActivePlayerRevision,sessionId:window.__OliviaSoulActiveSessionId,songId:window.__OliviaSoulActiveSongId,mediaUrl:p,event:"timeupdate",currentTime:t.currentTime,duration:c});if(p.includes("/toy/midi/songs/"))return;window.__OliviaSoulRememberDefaultPlayback(p,t.currentTime,t.loop),Z({event:"timeupdate",currentTime:t.currentTime,duration:c||a.value})}'
...
$stopTo = 'case"pause":case"stop":if(e.sessionId&&(e.sessionId!==window.__OliviaSoulActiveSessionId||e.songId!==window.__OliviaSoulActiveSongId))break;window.__OliviaSoulAbortPlayerRequests&&window.__OliviaSoulAbortPlayerRequests(),window.__OliviaSoulActivePlayerRevision=null,window.__OliviaSoulActiveSongId=null,window.__OliviaSoulActiveSessionId=null,e.cmd==="stop"&&e.restoreDefault===!1?i.value&&i.value.pause():window.__OliviaSoulRestoreDefaultPlayback();break;'
