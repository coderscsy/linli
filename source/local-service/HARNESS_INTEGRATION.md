...
当前正式服务默认使用根目录 `harness/`，版本由 `harness/VERSION` 标记，当前为 v18。正式目录只保留运行必需的栏目、预检、生成、检查、重写、写法与开信文件；`harness-v16-nofeel/`、`harness-v17/` 保留为历史实验配置，不由实时服务直接读取。
...
格式校验已经内置在 `harness-4step.ps1`：v18 正式预检必须为十三行；历史普通预检为八行，旧状态实验为九行。每行必须使用全角分隔符，最后一行必须是“结论”。格式不合格按模型调用失败处理，不能静默当作通过。
...
实时服务不再传入 `-SkipFeel`，脚本也不再接受该参数、调用 STEP2 或生成 `2feel` 兼容文件。草稿直接依据预检、栏目、写法、人设和分层往来生成。发布包不包含 `02-读信感.md`。
...
当前没有“重写后再检查”的第二轮闭环。若工程要求更强保证，应增加最多一次复检，并设置硬性循环上限，避免无限重写。
