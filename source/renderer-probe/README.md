...
- 以下来源均已在本机存在，且操作者有权只读访问：
  - 游戏根目录：`Z:\SteamLibrary\steamapps\common\BSide Olivia Lin Test`
  - Steam 库元数据根目录：`Z:\SteamLibrary\steamapps`
  - 备份根目录：`I:\Backups\BSide-Olivia-Lin-2026-08-31`
  - 游戏 AppData 根目录：`C:\Users\sycan\AppData\Roaming\miHoYo\Olivia-steam`

输入根目录始终只读；工具不会移动、删除、重命名、修补或启动其中任何文件。扫描仅以字节方式读取候选文件，不执行未知 EXE/DLL，不进行 DRM 绕过。

## 执行精确命令

从包含本工具的源码树运行以下命令（仓库工作树路径不同，仅替换第一行的 `Set-Location`，其余根目录和参数保持不变）：
...
唯一允许的运行时写入根目录是 `I:\OliviaSoulData\MidiRenderer`。扫描会在其下写入且只写入：
