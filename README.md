...
- 每日写信上限默认不限次数；可在“高级设置 → 今日信件次数”中设为 `0–999`，`0` 表示不限。
- DeepSeek 与本地 Gemma 两套模型档案独立保存，在管理页中手动启用。
- 示例本地接口：`http://127.0.0.1:8000/v1`；请按自己的 OpenAI 兼容服务修改地址和模型名。
...
社区用户提供了一个整理包，包含其备份的官方曲目和用户上传曲目：

- [百度网盘曲库整理包](https://pan.baidu.com/s/1sWJ3ppXnABt5Osvk3PFyZg?pwd=6666)（提取码：`6666`）

该链接及文件由社区用户提供，不属于本项目发布包，本项目未逐项核验内容、完整性或授权状态。请仅导入你有权保存和使用的曲目，并自行备份现有曲库。
